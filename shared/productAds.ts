...
export const UNIVERSAL_TEMPLATES: AdTemplate[] = [
  ...
  },
  {
    ...
    desc: 'The classic ad arc: frustration, then relief',
    beats: [
      { motion: 'hold on the problem', view: 'a relatable everyday frustration the product exists to fix — muted tones, tension in the framing' },
      { motion: 'the product enters decisively', view: 'it arrives center-frame and gets to work, the scene visibly turning' },
      { motion: 'push in on the resolution', view: 'the after-state: ease, order, a satisfied beat — the product hero in front' },
    ],
    light: 'cool flat light on the problem warming to bright optimistic light on the fix',
    audio: 'tense quiet opening resolving into a warm confident score',
    negatives: 'no mocking or distressing imagery of people, no warped faces or hands, no text',
  },
  {
    key: 'speed-ramp',
    label: 'Hype cut',
    desc: 'Speed ramps, flash cuts, drop energy',
    beats: [
      { motion: 'fast whip onto the product with a speed ramp to slow motion', view: 'the product snapping into frame, energy trail settling' },
      { motion: 'rapid-fire angle changes', view: 'three quick flashes of the product from different dramatic angles, each punchier than the last' },
      { motion: 'hard slam to the hero', view: 'the product locked dead-center on the final beat, a light flare kissing it' },
    ],
    light: 'high-contrast strobe-accented key on a dark set',
    audio: 'a bass-heavy trap/electronic beat, cuts landing exactly on the hits',
    negatives: 'motion blur never obscures the product on the holds, no text, no invented logos',
  },
  {
    key: 'family-lineup',
    label: 'The line-up',
    desc: 'The whole range, shoulder to shoulder',
    beats: [
      { motion: 'slow lateral tracking shot', view: 'the product range standing in a confident row, each catching the light in turn' },
      { motion: 'push in on the hero variant', view: 'the lead product steps forward in focus, siblings softly blurred beside it' },
      { motion: 'pull back to the full family', view: 'the whole line-up crisp and evenly lit, a poster-clean formation' },
    ],
    light: 'even premium studio light with a gentle sweep across the row',
    audio: 'confident modern score building to a unison final note',
    negatives: 'every variant stays exactly as photographed, no invented variants or labels, no text',
  },
  {
    key: 'retro-film',
    label: 'Retro film',
