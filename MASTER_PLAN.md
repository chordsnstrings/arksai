...
## 8. Execution status (autonomous run — updated)
- ✅ **Phase 1** (NODE_ENV devDeps leak), **Phase 2** (build any node stack on publish), **Phase 4** (truthful publish prompt **+ C1 runner anti‑thrash backstop**), **Phase 5** (grounded‑research loop — structural core + 9 tests) — implemented, gated (492 tests), shipped to `main` + auto‑deployed.
- ✅ **Phase 3 (subdomain serving + on‑demand wildcard TLS) — IMPLEMENTED, DEPLOYED, validated to the sandbox's limit.** DNS wildcard `*.apps → droplet` live. Caddyfile: `*.apps.arksai.studio` on‑demand‑TLS site + global `on_demand_tls { ask /internal/tls-check }`, apex block UNCHANGED — **validated with the real Caddy binary across CADDY_TLS = email/internal/unset (all valid), so the apex can't break.** Fastify: Host‑based root serving (any stack works at root) + the `/internal/tls-check` gate. **Live‑verified from the sandbox:** apex stayed 200 across the deploy, new code live, `tls-check` = 200 for a real slug / 404 for a fake one, path serving intact.
- ⏭ **Final verification needing a non‑sandbox network / a live build:**
  - **Live subdomain hit** (`https://<slug>.apps.arksai.studio` → cert issues + serves): the **sandbox egress allowlist blocks `*.apps.arksai.studio`** (only `arksai.studio` is allowed), so this must be confirmed from the **operator's browser** or the droplet console. Open `https://roasters-coffee.apps.arksai.studio/` (shows the "app not running" notice for that errored build, but a valid cert + the page proves the plumbing) — or publish a fresh app and open its subdomain.
  - **Phase 6 full e2e** (coffee prompt → builds clean → publishes → subdomain) needs a live agent build; the build/publish half is reachable via the `/apps/` path, the subdomain half via the operator's browser.
  - **Phase 5 live wiring** (`deps.llm`→MiniMax adapter, `deps.search`→web_search, invoke on the research family behind a flag) — the tested module is shipped; wiring + a live VC‑prompt run is the remaining seam.
