...
- B2B acquisition landing (pre-login) — positioned as an **enabler for UAE businesses** ("Put AI to work across your business"), with a prominent **"Built for the UAE"** compliance block (VAT 201/FAF, PINT AE e-invoicing, Corporate Tax, WPS, Excise), an **alpha · invite-only** status (nav pill + hero note), an early-adopter strip (the ARKS group of companies), a by-department value grid, FAQ, and invite-request capture (`POST /api/leads` → the operator accepts/rejects in the admin portal).
...
- **Per-org shared brain + agent-driven onboarding** — a new org admin gets a fully-visible, agent-led first run: ArksAI **crawls the org's website** (reads the brand colours + drafts an "about") **or reads an uploaded logo's colours** (`extract_palette`, deterministic — so it never claims it can't see the image), asks a few questions, and saves the org's shared profile (`save_org_profile`). That brand + "about" + a recalled digest of the org's own past chats are injected into **every** session in the org (brand applied consistently unless the user overrides), so the whole team shares one context. Members joining later inherit it silently; the admin can re-run/skip/edit.
