...
5. **Play Store track — PER-ORG developer accounts (scoped now, built next).** ArksAI is multi-tenant B2B, so Play publishing is NEVER through one platform-level ArksAI account — **each company connects its OWN Google Play Developer account** (their name on the listing, their $25, their legal ownership/billing). Design mirrors `org_email_accounts` exactly:
   - **`org_play_accounts` table** (org_id PK): the company's **Google Play service-account JSON** (a GCP service account they grant the *Play Android Publisher* role to in their own Play Console), **AES-256-GCM encrypted at rest** (`lib/crypto.encryptSecret`), + developer-account display name, default package prefix, `verified_at`. Connect/test/disconnect API is **org-admin-gated** (like the mailbox), write-only over the API.
   - **Per-app upload keystore** generated on first publish and stored encrypted per app (Play App Signing manages the final signing key; we hold only the upload key) so re-publishes use the same upload identity.
   - **Build = AAB** (`./gradlew bundleRelease`, alongside/instead of the debug APK) → **upload via the Google Play Android Publisher API** (`androidpublisher.edits`: upload bundle → assign to **internal → closed → production** track → commit) **authenticated with THAT org's service-account credentials** (`google-auth-library` JWT). The org picks the track from the page.
   - **Onboarding burden to surface in the UI:** the company must (a) have/createt a $25 Play Developer account, (b) create a GCP service account + enable the Android Publisher API, (c) invite that service account into their Play Console with publishing access. The page walks them through it; until connected, native builds still hand back a sideloadable APK (today's behaviour) and Play publish is greyed out.
   - **Dormant-until-connected**, exactly like the builder bake shipped dormant: no org account ⇒ no Play tab fires; web/APK unaffected. Gate the live upload test on a real connected org account.
...
**Build order:** (a) `/android` page + intake wizard ✅, (b) backend auto-deploy + deployment questions wired in ✅, (c) longer-build progress UX ✅, (d) Play Store pipeline — **per-org** account connect (`org_play_accounts` + encrypted service-account JSON + org-admin connect/test UI) → per-app upload keystore → AAB `bundleRelease` → Play Android Publisher upload to a chosen track, all authed with the org's own credentials (gate the live test on a real connected org account). iOS/EAS = separate later track.
...
- ✅ **SMALL-APP DONE-CONDITION MET (2026-06-22):** "Snap QR" built end-to-end on our droplet (expo prebuild + Gradle, no EAS) → a valid 90 MB APK (ML Kit barhopper QR scanner + CameraX + Hermes + Expo, 1046 files) → **installed on a real phone and runs fine (operator-confirmed).** Both the direct trigger and the agent-driven `build_apk` path produced the APK. First build ~18 min (cold Gradle cache — TODO: warm it in the snapshot to cut to ~3 min).
