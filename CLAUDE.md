...
- **Report quality lessons (real Flash run was bad)**: reports are floored to **ArksAI Pro** (never Flash) in the runner — Flash produced cover bleed, lonely pages, dark off-theme boxes, invisible text. Protocol now mandates: let content FLOW (don't force every section onto its own page → lonely pages), CONTRAST safety (no text same colour as bg/accent → invisible-text bug; callouts are LIGHT not dark), and ICONS that MUST be INLINED (external `<use href="icons.svg#x">` does NOT render in Chromium PDF). `add_fonts` also installs `icons.svg` (Lucide sprite) as an inline-source. The real fix for these visual bugs is the `see_image`/MiniMax-VL QC loop (needs MiniMax egress / fresh session). Core needs no MiniMax; vision QC (`see_image`) lights up when keyed.
