...
import { createBuild, getBuild } from '../../build/store';
import { startAndroidBuild, isBuildConfigured } from '../../build/androidBuild';

const POLL_MS = 12_000;
const MAX_WAIT_MS = 12 * 60 * 1000; // bounded; most builds finish in 3–7 min
...
export const buildApkTool: ToolDef = {
  ...
  async run(args, ctx) {
    ...
    for (;;) {
      if (ctx.signal.aborted) return `Build ${build.id} started but the run was cancelled; it will finish in the background.`;
      ...
      if (b.status === 'success') {
        ...
        return `✅ APK built${mb ? ` (${mb} MB)` : ''}. Download: ${url}\nShare this link with the user — they install it on Android (enable "install from unknown sources" for a sideloaded APK).`;
      }
      if (b.status === 'error') {
        return `❌ APK build failed (${b.phase || 'error'}): ${b.error || 'unknown error'}. The web/PWA build still works; fix the build issue (often a missing Expo module or a native dependency) and retry, or deliver the PWA.`;
      }
      if (Date.now() > deadline) {
        return `Build ${build.id} is taking longer than usual and is still running (${b.phase || 'building'}). It will keep building in the background; the download will be available at ${config.publicBaseUrl}/api/builds/${b.id}/download once done.`;
      }
