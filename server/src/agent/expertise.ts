...
const FIN_SHEET =
  'Financial-spreadsheet rigor: separate inputs/assumptions from calculations from outputs; put every assumption in its OWN labelled cell (never bury a number inside a formula) so a single change flows through. EVERY derived number MUST be a LIVE formula in generate_spreadsheet (pass {"f":"C5*(1+Assumptions!B5)","v":<result>} — formula + cached result — or a "=B2*C2" string; totals via =SUM(...)); a model that hard-codes its totals/balances/growth is rejected by the automated review. Compute every derived figure with compute_financials (operation dcf / ratios / variance / forecast) — never hand-type or guess a derived number; pass the computed result as the cached value. BUILD GRANULAR / multi-year monthly models IN STAGES via generate_spreadsheet: first the "Assumptions" sheet (every driver — rents, salaries, unit costs, escalation %), then call again with append:true to add the next 2-3 schedule sheets per call (CAPEX+OPEX, then Personnel+Summary/P&L), each referencing Assumptions with cross-sheet formulas — a few sheets per call is the sweet spot (far fewer round-trips than one-at-a-time, no truncation risk of one giant payload), and is how you deliver real month-by-month granularity. RESEARCH the real drivers first (comparable commercial rents, equipment prices, market salary bands) and cite them on the Assumptions sheet; never fabricate the numbers. Clean number formats (currency symbol, thousands separators, % where due); bold/shade the header row and totals, freeze the header; include a summary/totals; add light sanity checks. CROSS-SHEET REFERENCES MUST POINT AT THE VALUE CELL, never the rate/% cell — re-read the target sheet (read_spreadsheet) to confirm the exact row/column before you reference it; a formula that grabs a 0.30 growth-rate cell instead of the 6,000 revenue cell produces nonsense (a money cell under 1, a 0, or a 140-million total). NEVER put a section/divider row ("SPACE & LEASE", "—— REVENUE ——") inside a calc sheet — it shifts every row and breaks absolute references; one clean header on row 1, data from row 2, section names go in the TAB name. SANITY-CHECK the output before delivering: no derived cell should be 0 when its inputs aren\'t, no money cell should be under 1, no value should be wildly out of scale. Never invent figures. For a model generate_spreadsheet can\'t cleanly express — a true 3-statement model with circular links (interest↔debt↔cash), NPV/IRR/XIRR, VLOOKUP/INDEX-MATCH, or dynamic period logic — it is correct to ESCALATE to a Python/openpyxl script with REAL formulas in the cells (never dumped literals), then call recalc_spreadsheet ONCE to compute every value authoritatively and catch error cells; don\'t hand-loop soffice. A correct one-shot model is worth a longer build — a wrong-but-fast model is the failure.';
const HOME_SHEET =
...
const TASK: Record<string, string> = {
  ...
  'marketing.creative':
    `Ad & social creative: use generate_creative for each piece — a hero/ad plus channel-sized social graphics (1:1, 4:5, 9:16, 16:9). SPLIT THE BRIEF: the \`prompt\` is ONLY the imagery (subject, composition, style, mood, brand colour) with NO text/words in it; put the wording in the SEPARATE fields headline + subhead + bullets + cta (never cram the copy into the prompt — that's what makes image text garble). BRAND FIRST: ask the user to upload their logo + confirm the brand accent before building (pass the logo path, or set logo_placeholder if they have none). PRODUCT-LINK ADS: when the user gives a product/site URL, crawl_site + extract_palette FIRST — pull the real product name, benefit language and brand colours into the brief instead of inventing them. HOOK VARIANTS: for an ad, pass 2-3 alternate headlines in headline_variants (each a DIFFERENT proven hook archetype — question / bold claim / before-after / social proof / stat lead); you get an A/B/C set on the same background for one image cost — deliver all and let the user pick the winner. One idea per image, on-brand, high-contrast; expose every file as a download. ${SOCIAL}`,
  'marketing.storyad':
    'Story video ad: use generate_video_story (never chained generate_video calls) — it plans the scenes, keeps the product/cast/exact on-screen text consistent, and stitches one film. Craft: open on a hook (the problem or the wow) in the first two seconds; the product moment is its own scene (pass the product photo as a cast image, or exact UI/text via the story so it composites pixel-perfect); end on the payoff + one clear takeaway. Vertical 9:16 for Reels/TikTok unless told otherwise; ~20s default. ALWAYS show the returned scene plan and the stitched DRAFT before rendering the final; offer single-scene retakes by number.',
  'marketing.logo':
    'Logo & brand identity: use the generate_logo tool — it delivers a real designer submission (brand directive + palette + typography, a distinctive vector mark, LIGHT & DARK variants, app/website placements, and a zipped SVG+PNG+JPEG kit). INTAKE FIRST (one short round): the brand name + what it does, 1-3 concept directions to explore (or let it propose), a colour preference (or "you choose"), and the vibe. The mark is built CLEAN — real fonts for any letters and overlapping geometric primitives for any symbol, NEVER warped/morphed — so it reads at every size. Prefer a concept/pictorial mark when there is a drawable idea; a lettermark for abstract/name-led brands. After it returns, summarize the concept + palette in one line and offer to tweak colours/typeface or explore another direction. Do NOT hand-build a logo in HTML/CSS or use generate_creative for a logo.',
  'marketing.blog':
...
export const TASK_TRIGGERS: Record<string, string[]> = {
  ...
  'marketing.storyad': ['story video', 'video ad', 'multi-scene', 'story ad', 'video story', 'commercial video'],
  'marketing.logo': ['logo', 'brand logo', 'logo design', 'brand identity', 'brand mark', 'wordmark', 'visual identity', 'design a logo', 'make a logo', 'create a logo', 'rebrand'],
