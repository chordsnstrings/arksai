...
export function VideoStudio({ onClose }: { onClose: () => void }) {
  ...
  function explainerBrief(): string {
    return [
      ...
      '1. Write a RETENTION-FIRST script per MOTION.md "SCRIPT DOCTRINE": scene 1 is a HOOK (a question/bold claim/stake — never a title card or "in this video"); scenes chain with BUT/THEREFORE (never "and then"); one idea per sentence, concrete nouns, anchor stakes in "you", translate every number into a comparison the ear can hold; vary sentence lengths; no essay scaffolding/hype/CTAs (the tool rejects them); END on a ≤8-word punch-out that calls back to the hook. Pass target_seconds so the tool checks your word budget.',
